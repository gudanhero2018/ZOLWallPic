...
class Ui_Dialog(QDialog):
    ...
    def slot_startDownload(self):
        ...
        try:
            if url is None or url.strip() == "":
                ...
                html = Zol._get_html(url)
                soup = Zol._get_soup(html)
                #print(soup)
                if self.TYPE_MORE == type:
                    albums = Zol._get_img_dirs(soup)
                    if None == albums:
                        QMessageBox.information(self, "提示", "无法获取该网页下的相册内容，网页内容有可能发生了变化，请联系作者获取最新的程序！", QMessageBox.Yes)
                    else:
                    ...
                    title = Zol._get_page_title(soup)
                    #print(title)
                    QMessageBox.information(self, "提示", "已开始下载...", QMessageBox.Yes)
                    self.downloadButton.setDisabled(True)
            ...
            print(e)
            QMessageBox.information(self, "错误", "请检查‘爬取类型’和‘爬取地址’是否匹配，\n 多个相册（网页上有多个相册）\n 例如：http://desk..com.cn/1920x1080/ \n 单一相册（网页上单张照片页面）：\n http://desk..com.cn/bizhi/7498_92909_2.html ", QMessageBox.Yes)
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "爬取桌面壁纸网站"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), _translate("Dialog", "壁纸图"))
        self.downloadButton.setText(_translate("Dialog", "开始下载"))
        self.urlL_ineEdit.setText("http://desk.zol.com.cn/1920x1080/")
